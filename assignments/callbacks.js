...
sumNums(3, 4, function(sum){console.log(sum);})

function multiplyNums(x, y, cb) {
  return cb(x*y); // multiplyNums multiplies two numbers and passes the result to the callback.
}

multiplyNums(11, 5, function(product){console.log(product);})

function contains(item, list, cb) {
return cb(arr.includes(test)); // contains checks if an item is present inside of the given array/list.
